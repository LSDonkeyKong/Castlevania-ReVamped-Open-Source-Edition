...
if i_frames < 2 && global.timestop/2 = round(global.timestop/2)
{
	...
	if AI = GET
	{
		...
		if counter >= 60 && instance_number(objEnemyGremlin) < 8
			ai_lap()
	}
	
	if AI = POSITION
	{
		sprite_set(sprHarpyWithGremlin,1)
		scrViewData()
	...
	if AI = DROP
	{
		...
		if counter = 45
			instance_create(x,y + 16,objEnemyGremlin)
			
		if place_meeting(x,y,parSolid)
			y += 2
			
		if counter < 45
			sprite_set(sprHarpyWithGremlin,2)
		else
			sprite_set(sprHarpy,1)
